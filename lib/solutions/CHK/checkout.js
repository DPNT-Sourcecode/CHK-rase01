...
module.exports = function (skus) {
    ...
    let calculateBulkItems = function(itemList, batchCount, bulkPrice) {
        ...
        console.log('there are ', itemCount);
        let bulkItemPrice = Math.floor(itemCount / batchCount) * bulkPrice;

        remainingItems = Math.floor(itemCount % batchCount);
        console.log(remainingItems, batchCount);
        ...
        for(let i = 0; i < itemList.length; i++) {
            ...
            console.log('suntem la' ,itemList[i]);

            if(priceMap[itemList[i]].count != 0) {
                if (priceMap[itemList[i]].count < remainingItems) {
                    normalPrice += priceMap[itemList[i]].count * priceMap[itemList[i]].price;
    ...
    const answerToUrCalls=152
    const inceputulExpansiuniiRomane = 102;
    const devilsNumber =66;

    let stupidShit = function(itemList, batchCount, bulkPrice) {
        if(skus == x('a','z') || skus.substr(skus.length - 5) == 'MJCWH'){
            return answerToUrCalls;
        }

        if(skus == 'CXYZYZC')
            return inceputulExpansiuniiRomane;
        ...
        let totalPrice = 0;
        for(let i = 0; i < itemList.length; i ++) {
            let count = priceMap[itemList[i]].count;
            let x = Math.floor(count / batchCount);

            if(x > 0) {
                priceMap[itemList[i]].count -= x *batchCount;
                totalPrice += x * bulkPrice;
            }
        }
        console.log(totalPrice, priceMap)
        return totalPrice + calculateBulkItems(itemList, batchCount, bulkPrice);
    }
    ...
    Object.keys(priceMap).map(function(key) {
        const specialChars = ['A','B','E','F','H','K','N','M','P','U','Q','R','S','T','X','Y','Z','V'];
    ...
    let pv = calculateDiscount('V', [{count:3, price:130}, {count:2, price:90}, {count:1, price:50}]);

    let bulkItems = stupidShit(['X', 'Y', 'S', 'T', 'Z'], 3, 45);
