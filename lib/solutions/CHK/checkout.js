...
module.exports = function (skus) {
    ...
    let calculateFreeItems = function(buyItem, freeItemType, mustHave) {
        ...
        console.log(priceMap[buyItem].count, priceMap[buyItem].price);
    ...
    let calculateBulkItems = function(itemList, batchCount, bulkPrice) {
        ...
        for(let i = 0; i < itemList.length; i++) {
            ...
            if(priceMap[itemList[i]].count != 0) {
                console.log(itemList[i], priceMap[itemList[i]]);
                console.log('sunt ', remainingItems);


                normalPrice += remainingItems * priceMap[itemList[i]].price;
                console.log('pretul e ', normalPrice);

                remainingItems -= priceMap[itemList[i]].count;
                console.log(' au ramas ', remainingItems);
            }
        }
    ...
    Object.keys(priceMap).map(function(key) {
        const specialChars = ['A','B','E','F','H','K','N','M','P','Q','R','S','T','X','Y','Z','V'];
    ...
    return normalPrices + bulkItems + pe + pn + pr + pa + pb + pf + ph + pk + pp + pu + pv;
};
